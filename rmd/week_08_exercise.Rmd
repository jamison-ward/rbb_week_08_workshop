...
The first thing we need to improve on the previous bubble plot is the size range of the bubbles. `scale_size()` allows to set the size of the smallest and the biggest point using the `range` argument. We can modify the original plot object by adding new layers with `+`. Run the code below to set the size range of the points:
...
The original gapminder bubble plot we are recreating has a log-transformed scale on the x-axis. This helps by spreading out the points that are clumped on the left side of the plot.

In {ggplot2}, we can transform the x-axis using `scale_x_log10()`. Below, add this function as a new layer after a `+` sign:
...
gap_bubble04 <- gap_bubble03 +
  ...
  labs(title = "Global GDP and Life Expectancy in 2007",
...
gapbubble04_interactive <- ggplotly(gap_bubble04)
```
...
```{r}
# Save plotly plot
htmlwidgets::saveWidget(gapbubble04_interactive, file = here("rmd/images/gapbubble04_interactive.html"))
```
...
gap_bubble_time <- ggplot(data = gapminder_all,
                         mapping = aes(x = gdp_per_capita, y = life_expectancy)) +
  geom_point(aes(size = population,
                 color = continent),
             alpha = 0.5) +
  ...
  scale_x_log10(breaks = c(300, 1000, 3000, 10000, 30000, 100000),
                labels = label_number()) +
  scale_y_continuous(breaks = c(seq(20, 90, 10)), limits = c(20, 90)) +
...
# anim_save(gap_bubble_time, file = here("rmd/images/gap_bubble_time.gif"), dpi = 1080)

gapminder_animation <- animate(gap_bubble_time, nframes = 200, fps = 10, width = 3600, height = 3600, res = 600, renderer = gifski_renderer(here("rmd/images/gap_bubble_time_high_res.gif")))
